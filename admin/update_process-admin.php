...
require_once 'session.php';
		...
		<?php echo $locat = $_REQUEST['location']; ?> <?php 
		...
		if ($new_profie){
			...
			if(!is_dir($folder)){
				...
				if($image_flag){
						...
						if ($ahead_step) {					 
						echo $user_insert_query ="UPDATE `user` SET first_name='{$first_name}', last_name= '{$last_name}', email ='{$email}', user.password='{$confrim_password}', gender='{$gender}', date_of_birth='{$date_of_birth}', user_image='{$path_img}', address='{$address}', updated_at='{$time}' WHERE user_id='".$_REQUEST['user_id']."'";
