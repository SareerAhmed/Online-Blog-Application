<?php 
	...
	if (isset($_REQUEST['action']) && $_REQUEST['action']="view_post" ) {
		...
		WHERE post_status='Active' AND post.`post_id`=".$_REQUEST['post_id'];
...
<head>
	...
	<style>
		...
		#post_summary{
			...
			font-size:<?php echo $post_summary_font_size."px"; ?>;
		...
		a[href$='.pdf']:before {
			...
			a[href$='.png']:before {
			   content: url("icons/image-solid.svg");
			   margin-left: 1px;
			   display: inline-block;
			   width: 33px;
			   height: 33px;
			...
			a[href$='.docx']:before {
			   content: url("icons/file-word-solid.svg");
			   margin-left: 5px;
			   display: inline-block;
			   width: 33px;
			   height: 33px;
			...
			a[href$='.pptx']:before {
			   content: url("icons/file-powerpoint-solid.svg");
			   margin-left: 5px;
			   display: inline-block;
			   width: 33px;
			   height: 33px;
	...
	<div class="container-fluid">
		<div class="card hadow-lg p-3 mb-5 bg-body-tertiary rounded">
					...
					<div class="row">
									 	...
									 	if (isset($login_user_data['user_id'])) {
									 		...
									 		if ($result_cheack_following->num_rows>0) {
									 			...
									 			if ($follow_blog_data['status']=='Unfollowed') { ?>
									 				<td><a  href="following-process.php?page=post.php&action=refollow&blog_id=<?php echo $blog_id; ?>&post_id=<?php echo $blog_post['post_id']; ?>" class="btn m-2   btn-info" >Follow</a></td>
									 			<?php }
									 			if ($follow_blog_data['status']=='Followed') { ?>
									 				<td><a  href="following-process.php?page=post.php&action=unfollow&blog_id=<?php echo $blog_id; ?>&post_id=<?php echo $blog_post['post_id']; ?>" class="btn m-2  btn btn-dark" >Unfollow</a> </td>
									 			<?php }
									 		}else{  
									 			?>
									 			<td><a  href="following-process.php?page=post.php&action=follow&blog_id=<?php echo $blog_id; ?>&post_id=<?php echo $blog_post['post_id']; ?>" class="btn m-2 btn-info" >Follow</a></td>
									 			<?php
									 		}	
									 ?>
									 <td> 
						...
						<div class="col-3">
							<h5> 
								...
								<?php echo "<small>Blogger:   </small>".$blog_post['first_name']." ".$blog_post['last_name']; ?>
			...
			<div class="card-body">
			    ...
			    <hr>
			    <?php $decode_post = htmlspecialchars_decode($blog_post['post_description']); ?>
			    <div id="post_description">
			    <hp class="card-text" id="post_description"> <h3 id="post_description"><?php echo $decode_post; ?> </h3>
			    </hp>
...
<?php    
$select_all_attachments="SELECT * FROM `post_atachment` INNER JOIN `post` ON `post_atachment`.`post_id`=`post`.`post_id` WHERE `post_atachment`.`is_active`='Active' AND 
`post_atachment`.`post_id`=".$_REQUEST['post_id']; 
           $result_all_attachments = mysqli_query($connection, $select_all_attachments); 
           if ($result_all_attachments->num_rows>0) { ?>
           	<div class="container-fluid col-md-5 mt-5 mb-5">
	           	<div class="row height d-flex justify-content-center align-items-center"> 
	           		<div class="col-md-10">
	           			<h2 style="background-color: #B92533;color: white">Attachments</h2>
	           			<div class="card">
	           				<div class="p-3">
	           				<table class="table">
							  <thead>
							  ...
							  <tbody>
							      ...
							      <td> <a style="text-decoration:none;" href="<?php echo $post_attachment_path ?>" download  target=”_blank” > <?php echo $post_attachment_title ?></a> </td>
							    </tr> <?php }?>
							  </tbody>
							</table>
	           				</div>
